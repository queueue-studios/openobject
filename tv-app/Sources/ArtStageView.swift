...
// until the incoming is decoded, so there is no black gap between pieces); before the Host first answers
// it shows the Connecting mark, an empty rotation shows the splash, and sleep the dimmed drifting mark.
// This is the tvOS equivalent of player/public/display.js's stage.
...
// the wordmark reads at the same vertical proportion on the frame and on Apple TV (§13, Branding).
