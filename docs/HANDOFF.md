...
- **Landing-page favicon fixed (real files).** openobject.io was shipping a placeholder inline-SVG mark, not the brand. First tried the real favicon inlined as a base64 data URI; it deployed correctly (verified live with curl), but Safari kept showing the old cached icon (Safari caches favicons aggressively and is unreliable with data-URI favicons). Switched to real favicon files at the site root (favicon-16/32.png + apple-touch-icon.png), the same setup the player serves on openobject.local that Safari renders correctly. gh-pages now publishes those files alongside index.html, with CNAME preserved.
