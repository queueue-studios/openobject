...
- **A findable hint.** Settings > About carries the trigger sequence as invisible text after the license line (`.about-hint`, painted in the card's own `--panel` colour, `aria-hidden`), revealed only when the About text is drag-selected (a green `::selection` style). A discovery path for someone using the frame; the trigger is in the source regardless.
