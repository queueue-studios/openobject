...
| Video | MP4, MOV, WebM | **Always loop.** "Full length" or fixed duration (see §7). Audio follows the display **Sound** setting (default On), with a muted-autoplay fallback (§12). |
...
- **Connected** (§8) pieces hold for the duration like any other piece; with **Animate** on, the piece's own motion runs continuously to fill it. The rotation times each piece's slot from when it actually appears on screen, not from when its render is kicked off (fixed 2026-06-17). This matters only for connected pieces: their generative sketch runs a heavy **synchronous** `generate()` inside a **same-origin** iframe, which briefly blocks the shared render thread (about 4 to 5 seconds on the frame's N5105). Timed from the render start, a connected piece was on screen for only *(duration minus generate time)*, roughly 7 to 8 seconds of a 12 second setting; the visible piece *before* it lingered by the same amount. Reveal-gated timing gives every piece its full on-screen duration. The "appears on screen" signal is the iframe's `load` event, which fires when `generate()` finishes; connected pieces therefore use a **long safety fallback** instead of the brief one files use (`render()` in display.js), because a 500ms fallback would win the race on a slow frame and start the clock mid-generate, the very thing this fixes (refined 2026-06-17). That signal is right for a sketch that renders synchronously and wrong for one that loads its own assets first: p5 waits for that very `load` event before it constructs the sketch, and only then runs `preload()` and `setup()`. A piece like *Lost in Moffat County* therefore revealed into an empty document and held a **black stage** for about 1.7 seconds on a Mac (longer on the frame) while its images decoded. A collection can now set **`awaitPaint`** (registry flag, off by default), which reveals on the sketch's **first painted frame** instead: the display polls the same-origin mirror for a canvas plus p5's global `frameCount >= 2`, so the outgoing piece holds the stage until the incoming one is genuinely ready and the stage never goes black. A hidden page (a background tab on a computer, never the kiosk) suspends `requestAnimationFrame` and so cannot paint, and reverts to the load event rather than stall the rotation; a 12-second cap and the existing 30-second backstop bound the wait either way. It reads the bundle rather than changing it, so it works with pieces mirrored before it shipped (added 2026-08-09, §20). The unavoidable generate stall now lands between pieces (it freezes the outgoing piece for a beat, invisible when that piece is a still). A future option is to keep connected iframes warm so `generate()` runs once instead of on every appearance.
...
**The first survey, on the WRONG surface.** Matt tested all 14 Connected Collections plus an uploaded SVG piece on 2026-08-05 and found thirteen of fourteen correct, with only Golden Lining showing its grey/black tile corruption (§20 2026-06-30). That already falsified §2's blanket rule. **But that run was Mac Safari, not the target device**, which nobody uses as a display surface: the Mac app drives real Chrome precisely because of this bug. A failure there predicts nothing about iPadOS.

**The second survey, on the real device: Golden Lining renders PERFECTLY (2026-08-06).** Over a two-hour run on an iPad in Safari, **in landscape** (the large-surface worst case for this bug), roughly nine full cycles of the whole shelf: Golden Lining was clean on every pass, and Matt reported no rendering problems with anything else. This is consistent with the recorded root cause rather than contradicting it: the corruption was always described as canvas-size dependent and *worse on the Mac, because a bigger canvas means more tiles*. A 5:4 piece on an iPad never reaches that surface size.

**So the whole WebKit exception may not exist on the platforms that matter.** Provisionally retired, pending the two checks below: the skip mechanism, the "Chrome Only" pill, and the pre-rendered Golden Lining video. All three were designed to cover a failure that does not appear on the target device. The reasoning behind each is kept below because it is the right answer if any *future* piece fails, but none of it should be built on current evidence.

**Still open before calling it settled: the iPhone.** The app is Universal, so the conclusion has to hold there too. A smaller screen means less tile pressure, so the expectation is fine, but the original 2026-06-30 report specifically named the iPhone, and that report is now suspect in exactly the way the Mac one was. Note that running the *shipped* app on an iPhone does not answer this: it renders no Connected art at all, so the WebKit question is untouched by it.

**inkField cleared 2026-09-01** (Matt: treat anything tied to inkField as done). It had been the other open check here, absent from the surveyed rotation because the Mac app bundles a snapshot of `player/` from build time and his predated inkField's 2026-07-29 addition. As the heaviest piece on the shelf it was the likeliest exception, so closing it removes the larger of the two doubts.

**The SVG comes along for free.** `CapabilityFilter` skips SVG natively because `ImageIO` would freeze it on frame one, breaking the never-freeze rule (§6). A web view animates it properly, so the same path that rescues the 13 also rescues SVG uploads on iPhone and iPad. WebM remains out (an unrelated licensing question, not a rendering one).

**The path.** Point a `WKWebView` at the Host's own same-origin mirror using the identical URL contract `player/public/display.js` already builds: `/collections/<slug>[/<token>]/index.html` plus the per-piece seed (query *and* `#fragment`, which must not be conflated), `rpc_url`, `ooanim`, `oospeed`, `oochoice`, and the general `oo_<key>` controls, with the `crop` and `aspect` sizing rules ported. The web view needs inline playback and autoplay without a user gesture. Nothing new is required of the Host.

**Golden Lining is skipped, and skip comes first.** The skip mechanism is the safety net that makes the whole path shippable, and it is needed regardless of whether the pre-rendered video is ever built: a video may not exist yet, may not be cached yet, and any *future* piece that breaks on WebKit needs the same lever with no App Store cycle in between. Behaviorally it is the house pattern already: `RotationEngine` filters the raw list once (`rawItems.filter { $0.isRenderable(using: filter) }`) and advances only within the survivors, so the piece's turn never arrives. This is not new machinery; it is what the tvOS app does today with all 14 collections.

**Where the flag lives.** `CapabilityFilter`'s standing principle is that the app decides from what it can DECODE, never from a flag the Host might not send (§11). "Golden Lining corrupts on WebKit" is curated per-collection knowledge that cannot be derived from decoding, so it breaks that rule either way. Resolution: **the app carries the known-bad list AND honors a Host-sent flag, taking the union.** The app is therefore safe against any Host, old or unaware, while a newly-discovered bad piece can still be fixed with a one-line `collections.js` edit instead of a release. The app never *depends* on the flag; the flag can only add to what the app already knows. One prerequisite: `DisplayItem` decodes `id`, `kind`, `format`, `fit`, `filename`, `src` but not `collection`, which the Host already sends on connected items.

**Control panel: a "Chrome Only" pill.** On the Settings > Connected Collections row, on the existing `.cc-sub` line (`by <artist> · N pieces`), driven by the same flag so the panel and the apps cannot drift. Naming was worked through and the rejected options are worth keeping: **"Limited"** reads as a defect in the artwork rather than in one browser engine, which is the wrong tone for a curated artist's shelf; a struck-through **"iOS"** fails by *inversion* rather than confusion, since a missed line-through leaves a pill that asserts the opposite, and strikethrough carries no meaning to assistive tech; **"Needs Chrome"** implies an action that does not exist on the device where the pill matters most, because every iOS browser is WebKit underneath. **"Chrome Only"** makes no promise, classifies rather than instructs, and matches the noun-label grammar of the existing pills (Connected, Fill, Folder, SVG). Naming the frame was considered and dropped: the no-frame Mac path is the hero, so pill real estate should not go to the advanced path. **This pill cannot ship before the WebKit path**, since today no Connected piece plays on any viewer app and singling out Golden Lining would be wrong.

**Endurance measured, and it does not gate the feature (2026-08-06).** The other worry was that live p5 and three.js sketches would cost too much to run for hours, which would have meant designing in a quality cap or a fallback before committing. Measured on an iPad in Safari, unplugged, all Connected pieces at one minute each, two hours, room at 78F, **with a leather cover over the back** (so a pessimistic thermal setup): battery **73% to 37%**, about **18% per hour**, extrapolating to roughly 5.5 hours from full, or near twice the cost of plain video playback. The device ran **warm but not hot**, and **motion still looked as smooth at hour two as at the start**, so it was not thermally throttling. Auto-brightness drifted 50% to about 40%, which is either the room or a first-stage thermal mitigation and was not resolved; it also means the drain figure is slightly flattered.

**Conclusion: no power-management design is needed.** Plugged in this is a non-issue, and mains power is the expected deployment anyway (the frame, the Mac, and the Apple TV are all plugged in; a wall display is inherently mains-powered). Unplugged it is a several-hour device, which is fine for casual use and not a permanent installation. Two caveats keep the number honest in opposite directions: inkField was absent (optimistic), and Safari suppressed the audio (also optimistic), while the leather cover made the thermal read pessimistic.

**Audio is a Safari artifact, not a platform limit.** During that run The Bloom played silently even with both the display Sound setting and the collection's own Music control on. Expected: its score is a plain `<audio>` element that our `bloom` hook starts programmatically, and iOS Safari refuses script-initiated playback with no user gesture. The frame's Chromium kiosk and the Mac app's Chrome both pass `--autoplay-policy=no-user-gesture-required`; Safari has no equivalent and none can be passed. **A shipped app would not have this problem**: `WKWebView` exposes `mediaTypesRequiringUserActionForPlayback`, which set to empty is the direct equivalent of that flag. Do not record the silence as evidence against the WebKit path.

**A correction worth keeping, on how this was nearly got wrong.** An earlier draft of this entry called for a `WKWebView` prototype on a real device, and Claude removed it on the reasoning that mobile Safari and `WKWebView` are the same engine so rendering was already settled. That reasoning was sound and the conclusion was wrong, because the variable that mattered was never Safari-vs-`WKWebView`: it was **canvas size**, and the survey it leaned on had run on a Mac. The on-device check was the one that changed the answer. When a bug's recorded root cause names a variable (here, surface size), test that variable rather than the one that seems more obviously different.

The one thing Safari cannot answer is app memory budget, since a `WKWebView` hosted in an app has a tighter one than Safari's own process. That is a smaller risk and is fine to discover during the build.
...
**Send/Receive offline (E30, built 2026-09-17, §20).** The one piece that read the chain while it rendered
now gets its state captured once, at Add, beside its bundle, and is answered from that file by the Host and
by the copy alike: no network after Add, like every other Connected piece (Matt's rule, the same day). The
design record is the §20 entry.
...
**Scope this narrowly.** This is *not* the general "the Host pre-renders every Connected piece" idea recorded in `docs/TVOS-APP-PLAN.md` §2, which Matt has set aside. It is a targeted workaround for the handful of pieces that fail on WebKit, and it was **gated on two conditions**: (1) a Safari survey shows only a few incompatibilities, so a per-piece fallback is proportionate rather than a de facto second rendering path, and (2) the video is judged a fair representation of the work.

**Very likely unnecessary (2026-08-06). Do not build this without re-reading the entry above.** The purpose of this video was to cover Golden Lining on the viewer apps. On 2026-08-06 the piece was found to render **perfectly on an iPad in Safari**, in landscape, across roughly nine cycles: the corruption is canvas-size dependent and appears on a Mac-sized surface, not an iPad one. The 2026-08-05 survey that motivated this entry had run in **Mac Safari**, which is not a display surface. So there is currently no platform that both needs this piece and cannot render it: Apple TV cannot use a pre-rendered video either, since that route was set aside, and Mac and frame both run Chromium and are unaffected. Left on record because the reasoning and the render spec are correct if a genuinely WebKit-broken piece ever appears; retire it for good once inkField and the iPhone are checked.
...
### Prebuilt release image: closed for want of hardware to validate on (noted 2026-08-08)

The design record for **E7** (the single-file USB installer as a GitHub Release asset) and for the **E20** split that came out of it. E7 was the biggest revivability item on the list, the difference between "follow a technical guide" and "flash one file". It is **closed off the roadmap** (`docs/ROADMAP.md` D16, 2026-08-08): its doable half was split out as **E20** first, and what remained could never become actionable, since validating it needs a second XXL to wipe and Matt does not expect ever to have one. This record is kept whole rather than trimmed, so a stranded owner who does have a spare unit can pick it up from here.

**The manufacturer-variance worry points at the wrong layer.** The natural fear is that Infinite Objects shipped units with different software (a different Ubuntu, a different White Walls build). That is a non-issue: the install **wipes the machine**, replacing Ubuntu and the Waydroid container wholesale with minimal Debian, so every unit converges the moment the installer runs. What actually varies is **hardware and firmware**: BIOS version, eMMC size and device naming, RAM, and above all whether every XXL used the same mini PC across production runs. Matt's reports board name `XXL` on a MeLE Quieter 3Q, which reads like a commissioned SKU, but **one unit is the entire sample**.

**If it is ever built, it is an automated installer ISO, not a disk clone.** A prebuilt disk image (clone a working eMMC, owner flashes it) was considered and is the wrong shape: it bakes in one partition layout and disk size, needs first-boot resize machinery, and, worse, ships one machine's identity, so every frame in the world would boot with the same SSH host keys and machine-id from a public Release asset. A **customized Debian installer ISO** (Debian's own installer driven by an answer file, then our `install.sh`) keeps disk detection, partitioning, firmware and driver selection in Debian's hands, which is tested against orders of magnitude more hardware than this project ever could be, and generates per-machine keys normally.

**`install.sh` is already the robust half and must not be forked.** It hardcodes no disk, no partition, no `/dev/mmcblk0`; it is packages, systemd units, hostname, Avahi and Wi-Fi power-save, and it already absorbs Debian release differences (falling back from `polkitd` to `policykit-1`). The ISO would run **that same script from the same repo**, exactly as the player is not forked per platform.

**Why it is parked: there is nothing to validate it on.** A VM proves the automation logic and nothing else. The failure surfaces that would actually differ on another owner's unit are the ones a VM cannot see: `i915` driving the real 1920x1920 panel, the `iwlwifi` module and its firmware, the BIOS boot entry, and eMMC device naming. Installing to a **USB SSD** instead of the eMMC was identified as a zero-risk way to prove all of that on the real frame (the unit already boots from USB, §4), and was then **ruled out by Matt on 2026-08-08: the frame is considered done and untouchable, and no installer USB is to be introduced for any purpose.** With that closed, no validation path exists.

**And it should not ship unvalidated, because the automated part is the destructive part.** A bug in the answer file does not produce a cosmetic glitch; it wipes an eMMC and then fails to boot. The current manual procedure is **proven on real XXL hardware**, so replacing it with an unproven path, for people who get one shot and may have no way back, is a reliability downgrade dressed as a convenience upgrade. That runs against the mission rather than serving it.

**What would reopen it:** a second unit to test on, or a stranded owner willing to be the first while the manual path stays documented. Neither is expected, which is why the row is closed rather than parked. That owner's risk calculus genuinely differs, since their frame is already dead to them as an art frame, so a failed install leaves them roughly where they started **provided the backup step happened**. That is an argument for building it eventually, not for shipping it blind.

**Two requirements to carry forward if it is ever built.** They are recorded here because both are easy to lose in an unattended installer. **(1) Never target "the first disk".** The eMMC must be identified explicitly and confirmed, with model and size shown, before anything is erased. This is what would make a USB-target test safe, and it is what stops the stick wiping a laptop it was booted on by mistake. **(2) Never silently skip the pre-wipe backup** (§4 step 6). That image is the owner's only route back to the original White Walls install while the vendor's servers still exist (§18), and an unattended installer would sail straight past it. Prompt for it; do not assume.

**The split (E20).** Most of what makes the current install laborious is not the destructive step, and all of it is testable in a VM with no contact with any frame: ship the checkout as a **Release asset** (a `git bundle` or tarball) so seeding the code becomes a download rather than a USB dance; a **one-command bootstrap** that fetches and runs `install.sh`, which runs after Debian is installed and so never touches partitioning; and **sharper Debian-installer instructions**, the exact screens and choices, which is where a non-technical owner actually stalls. That is E20, and it is available now.

**Documentation plan, whenever either lands.** The Setup Guide leads with the simplest path that exists and keeps the manual procedure as an appendix, **never deleted**: it is the fallback when a preflight refuses an unrecognized unit, when the image is unavailable or broken, when someone is installing on non-XXL hardware, and it is the path that works with none of our infrastructure, which the mission requires. HANDOFF §4 stays the engineering runbook either way.

### Offline / portable playback on the viewer apps (noted 2026-07-28; iPad design settled 2026-09-15)
...
**The driving use case (Matt, 2026-08-08).** A gallerist loads a piece at home, unplugs the device, carries it to the gallery, and plugs it into a TV there with no network of any kind. That workflow is the target, and it is what the scope below is judged against.

**Some of this already works, by side effect.** Both apps play cache-first, so a *running* session keeps cycling already-seen pieces even if the Host drops off Wi-Fi. That is a property of the media cache rather than a feature, and three things keep it from being a real "load it at home, then take it off the network" mode: the cache is **bounded** (512 MB LRU), so a larger rotation evicts its earlier pieces; it lives in **`Library/Caches`**, which the OS may purge at any time; and the **rotation list is held only in memory** from the last poll, so a relaunch while offline has nothing to play and sits on Connecting.

**What a genuine portable mode needs.** The rotation manifest **persisted** outside memory; a real **non-purgeable offline store** kept separate from the cache; and an **offline launch path** that plays the saved set instead of waiting to connect. This note first also asked for an explicit **"Download for offline"** choice, on the reasoning that the app stays a viewer and does not own art (`TVOS-APP-PLAN.md` §3, and a cache is not ownership). The settled design below drops the choice and keeps the principle: the copy is made **automatically**, and it is still a mirror of the Host's rotation, never a library of its own.

**Apple TV cannot do this, and the reason is storage, not the network (checked 2026-08-08).** Worth stating precisely, because the intuitive blocker is the wrong one and this will otherwise be re-litigated on parity grounds.

- **The network is not the problem.** An Apple TV needs a connection for setup and activation, but an installed app launches and runs offline afterwards; Apple's own downloaded-video and Arcade cases depend on that. A cold Apple TV plugged into a gallery TV with no Wi-Fi is fine in principle.
- **tvOS guarantees an app 500 KB of persistent storage.** Anything larger goes in `Library/Caches`, and Apple's own wording is that when space is low and the app is **not running**, that data may be deleted. There is no non-purgeable store to opt into, and app plus cache together cannot exceed 4 GB.
- **On-Demand Resources do not rescue it.** ODR assets are authored at build time and hosted on the App Store, so an owner's uploaded art can never be an ODR asset, and ODR fetches over the network anyway, which is the very thing the feature is meant to survive.
- **The failure mode is the worst available.** "Unplugged, in a bag, app not running" is exactly the state in which tvOS is permitted to purge. The art would load at home and then be gone on arrival, with no network and no recourse: a blank wall at an opening. A feature cannot ship with that as its unlucky path.
- **Precedent.** Riley Testut has cited the lack of persistent storage as a major reason Delta was never ported to Apple TV. User-supplied ROMs are the same problem as user-supplied art.
- **Only the bytes are affected.** A remembered Host fits well inside 500 KB, which is why `UserDefaultsHostStore` works fine on tvOS today. This limit constrains holding media, nothing else.

**iOS is where the use case actually lands.** iOS has real persistent storage, and an iPad is the device someone would carry anyway. Two shapes, in build order: the **iPad as the display** on a stand (portable, no network, nothing else required), and the **iPad feeding a TV over a USB-C-to-HDMI adapter**, mirroring the full-screen art. The second would give the gallerist the big screen with no network at all and is the more interesting answer, but it is **untested**: treat it as promising, and verify mirroring on real hardware before it is described anywhere owner-facing.
...
- **Launch and the offline path: opening the app is the whole gesture.** With a local copy on the device, the stage starts **from the copy at once** and polls the Host in the background: no Connecting beat, no picker, nothing to tap. A Host that answers folds its live response in through the engine as today (`RotationEngine.apply` restarts nothing). The remembered-Host watchdog (Connecting for ~8 s, then the picker) applies only when there is **no** copy. A live Host that drops mid-session now **keeps rotating from the copy** rather than only holding its last frame (a strict improvement on `TVOS-APP-PLAN.md` §16). Tapping the local-copy row in the picker takes the same path, since it is simply `select(host)` for a Host that happens to be absent.
- **The Demo Gallery is excluded, with no special handling beyond "never saved".** Choosing it is deliberately non-persisting (`TVOS-APP-PLAN.md` §12), so it never becomes the remembered Host whose rotation the copy follows; it is public and online by nature, so anyone who can reach it needs no copy and anyone who cannot is not offered it; and it is demo art, not the owner's, so silently holding gigabytes of it by default would be the wrong default. Gallery sessions keep today's small purgeable cache. Carrying the demo art offline for showing the app is a separate decision, not taken; its smallest form would be a press-and-hold on the Gallery row.
- **Where and what.** The copy lives in **Application Support**, which iOS never purges, marked **excluded from iCloud backup** (the art is re-fetchable from the Host, and a multi-gigabyte backup would be rude). Today's purgeable LRU cache in `Library/Caches` stays, for the Gallery and as the decode path. What is held: the remembered Host's rotation **exactly as `/api/display` returns it**, uploads and Folder items alike (one item shape; `MediaPipeline.mediaURL` resolves both), Connected pieces skipped as today, plus the **last successful response saved as the manifest** so a relaunch knows the items, order, mode, duration, and Fit per piece. Sleep hours do not block saving: the Host sends its item list alongside `asleep`.
- **One Host at a time, the remembered one.** Choosing a different Host drops the previous copy and starts saving the new one. The copy is always exactly one Host's current rotation, never an accumulation.
- **Reconcile on every successful poll.** New pieces are saved; departed pieces are deleted after a **grace period** rather than at once, so a brief Pin or a moment's edit does not churn gigabytes: the Host collapses `items` to the pinned piece on the wire, which without the grace would delete the rest of the rotation and re-fetch it on unpin.
- **Space: as much as the rotation needs, never past a free-space reserve, no fixed ceiling.** Unlike the frame's Folder cache (a 20 GB cap protecting the Library), the iPad has no Library to protect; the only competitors for space are other apps and the OS, and the reserve is what protects them. The reserve is **one tenth of the disk, with a 5 GB floor** for small devices (a 512 GB iPad keeps ~51 GB free; a 64 GB iPhone ~6 GB), tunable at build, never a setting. It is **checked before every file**: a piece that would breach it is skipped and the fill moves on, so one oversize video never blocks the stills behind it, and if other apps fill the disk later the app simply stops saving until room returns. It never deletes anything but its own copy. Offline playback plays what fit and skips the rest, never blanking; the next poll resumes filling when room appears.
...
- **Offline rotation controls (settled 2026-09-16, Matt; roadmap E26).** Two rotation settings become changeable on the iPad, **only while it is playing its local copy**: **Order** (Sequence / Shuffle) and **Every** (the duration), the control panel's own words. *The model:* an offline change lasts until the iPad sees the frame again. While the frame is live its values apply and the controls are not shown, which by itself says where settings normally live; the change survives relaunches while away, so a venue setting does not reset overnight, and clears the moment the frame answers (its values were captured fresh then anyway). No sync back to the frame: the iPad stays a display. *The UI:* the tap-revealed overlay's one offline-only element, the "Playing local copy" capsule, is the entry point. Offline it gains a chevron; tapping it drops a small panel beneath, same material, two rows: Order as a two-segment control, Every as a menu of presets (15 s, 30 s, 1 min, 2 min, 5 min, 10 min, 30 min, 1 hour) whose label shows the current value, a non-preset value such as 45 s included until one is picked. Both apply at once (Order reshuffles from the next piece, Every re-times from the piece on screen), no Save or Done, no explanatory text: appearing only offline is the explanation. Tapping the art hides the overlay as today. When the frame is live nothing new appears anywhere. *Rejected:* a third, always-inline capsule at the bottom of the overlay (louder, tight on an iPhone). *Sound stays where it is:* the iPad already owns its Sound setting (the picker's speaker, sticky, offline-capable, separate from the frame) and the device's volume is the loudness control; it is not a rotation setting, and duplicating it here was declined. *Build shape:* a small persisted override (duration, mode) held with the local copy, applied on top of the seed and whenever the Host stops answering, cleared on the next successful poll; `RotationEngine.apply` already folds a duration or mode change in without restarting the piece.
...
**On parity across the three devices.** The parity that matters, and that already holds, is **rendering**: all three apps show a Host's rotation identically. Becoming a standalone art store is a different capability, and tvOS declines it at the platform level, so an iOS-only offline mode is Apple's asymmetry rather than ours. Revisit only if Apple ships a real persistent-storage allowance for tvOS apps.

### Folder Collections (Phase A local BUILT 2026-07-08, §20; Phase B frame BUILT 2026-07-11, §20; Apple TV BUILT: no folder-specific code needed, verified Phase C5.4)
...
**Built (Apple TV) with no folder-specific code, verified Phase C5.4 (2026-08-08 note).** The "thin native tvOS folder client" sketched below was never needed as a separate build, because the Host resolves a folder *before* it answers: `folders.itemsFor()` returns items in the same shape as Library items (`id / filename / format / kind / fit / src`), and a folder item's host-relative `src` resolves through `MediaPipeline.mediaURL` exactly as an upload's `/uploads/<file>` does. The app cannot tell the difference, which is the whole point of the one source contract. Confirmed in the tvOS build (`TVOS-APP-PLAN.md` §5, §11) and on Matt's real Apple TV. The bullets below therefore read as design history for the reach decision, not as pending work.
...
- **Phase B, the frame (built 2026-07-11) and Apple TV (built, no folder-specific code).** The folder lives on the owner's Mac; the XXL frame (or an Apple TV) is the display. The reach is settled: the **OpenObject app runs on the Mac and serves the folder read-only over HTTP** on the LAN, and every screen reads it over that one rail. The frame keeps a **bounded, ephemeral cache** (a session buffer, wiped on reboot) so playback stays local and smooth **without a Folder Collection ever crowding out the Library** or wearing the eMMC. This honors §9 (the network is never in the playback path). Phase A and Phase B share one **source contract** (a folder yields a list of items plus an HTTP media URL per item), so the display code is identical across phases; Phase B changes only where the bytes come from and how much is held locally (see the design below).
...
- **Order** follows the **Rotation** tab's Sequence/Shuffle (the §7 set; no Random, dropped as before), the same global control a folder already borrows for its duration (a build change 2026-07-11: the folder no longer carries its own order). Each display point applies its own Rotation order to the folder, so a frame and the Mac can sequence the same folder differently; leaving folder mode restores the Library's rotation untouched.
...
**Data model.** A `folder_collections` table (`path`, `name` defaulting to the folder's own name, `artist`, `fit`; the `order` column is retained but unused as of 2026-07-11, since a folder plays in the Rotation order) plus one `display_source` setting (`library`, the default, or a folder id) that is the either/or switch. No per-folder duration column (global), no include column (always on). `/api/display` branches on `display_source`: for an active folder it returns that folder's compliant files in the same item shape the display already consumes (a synthetic key per file, `format`/`kind` from the allowlist, the folder's `fit`, and a media URL under a per-folder route such as `/folder-media/<id>/<file>`) and the global Rotation `mode` (the folder's own `order` is no longer used, 2026-07-11); the Library path is unchanged. The **media route resolves by folder id plus file key**, pointing at the local folder in Phase A and at the frame's local cache in Phase B, so `scan folder to items` becomes `read the cache manifest to items` with the display none the wiser (whatever is not yet cached is fetched on demand and folded in). An unreachable-source status slot exists from the start (inert locally; on the frame an unreachable folder keeps showing the last synced copy and retries, rather than blanking).
...
**Phase B design (settled 2026-07-10, §20).** It follows from one stated priority: **make the non-XXL experience (a Mac, and above all an Apple TV) easy and slick; the XXL owner is the edge case, and the UI must not be complicated to accommodate the frame** (Matt, 2026-07-10). Each decision below is downstream of that.

- **The app is the one server, for every screen.** An Apple TV can only be fed over HTTP (tvOS has no way to read a file share), so the OpenObject app must run on the Mac to serve a folder to a TV at all. Rather than build a second, different path for the frame, **the app serves the frame the same way it serves the TV**: Mac, frame, and Apple TV all read the folder over the app's read-only HTTP on the LAN. A mounted **SMB** share was explored as the "no app for the frame" route and **dropped**, along with its guest-access and stored-credential variants: guest SMB is a fading, off-by-default macOS setting (a poor next-owner bet), and a stored login is a trust cost (a small proprietary app holding a Mac credential). App-HTTP holds **no credential** anywhere and does not rot. The consequence, explicitly accepted (Matt, 2026-07-10): **a Folder Collection only works while the Mac and its app are alive**; a reboot or a quit simply stops the folder display, which is obvious and fine for a feature used occasionally and left running. This retires §8's SMB-pull seam for this purpose.
- **The app is the one home for the definition, so the frame never picks a folder.** A Folder Collection is defined once, in the **app's** control panel on the Mac; because that engine is a Mac, its **native OS folder dialog is the only picker** (the very Phase A picker that already works). Every other screen only **selects**: the frame's Rotation **Source** dropdown lists the folders the app is serving and the owner picks one. This resolves the picker question by removing it. The Phase A frame-side "Add folder" (which could only browse the frame's own empty disk) is **removed**, and the sandboxed in-browser browser is not needed on the frame at all.
- **Control Panel split.** Folder management (add via the native dialog, edit Name/Artist/Fit, remove) lives in the **app's** control panel; the **frame's** control panel is **select-only** (the Phase A Source dropdown, populated from the app over the LAN). Setup on the Mac, selection from any screen. This keeps the frame's UI a hair simpler than the Mac's, never more complex, as the priority demands.
- **The frame cache is an ephemeral session buffer, not a persistent mirror** (this supersedes the hard-capped persistent-cache sketch that this paragraph used to describe). While a folder is the live source the frame **prefetches upcoming clips and accumulates** what it fetches (each file written to the eMMC at most once, kind to the flash across a months-long run), growing until the whole folder is local **or** a budget cap is hit (**default ~20 GB, plus a free-space floor** so it always yields to the Library and the OS) **or** the owner leaves folder mode. It is **wiped on reboot, on switching to the Library or another folder, and by a manual Clear cache control.** Nothing folder-related persists at rest, so no storage is reserved for a feature that may go unused, and the Library (the hero, §9) can never be crowded. Because only one folder is ever live (the either/or), the buffer only serves the active folder's working set; if the network cannot keep pace, lag appears at the buffer, never a frozen screen. This drops the aggregate-cap, least-recently-shown eviction, per-folder discard-flag, and age-out machinery once weighed here: with nothing persisting past a session, the only controls needed are the automatic wipes and the manual Clear.
- **Error states (no dead clicks), as-built 2026-07-11.** **(1) No Mac available** when selecting a folder (the app is not installed, or not running; the frame cannot tell these apart, both simply mean no host is advertising folders): the empty Source shows a brief *"Looking for your Mac…"* while discovery warms up, then settles into *"To display a Folder Collection, the OpenObject app must be open on your Mac."* Once a Mac starts sharing, the folder appears on its own (the frame keeps polling), no refresh. **(2) The Mac is present but too old** to serve folders: the frame shows *"A newer version of OpenObject is available for your Mac. Please update the app."* (the general version check above), rather than mistaking an outdated Mac for an absent one. **(3) A live folder goes unreachable** (the Mac sleeps, the app quits, the Mac is shut off): the folder keeps its place in the Source, greyed with its name, and reads *"Mac unreachable. Make sure it is awake and the OpenObject app is open,"* while the frame plays on from cache, so a mid-run stop is explained, never silent. These states are frame/TV-only; a Mac-as-display owner's engine and folder are the same machine, so nothing is ever on a network to be unreachable.
- **Mac availability while serving (the sleep question, Matt 2026-07-10).** A folder plays only while the Mac can serve it, but softer than it sounds. The cache means the Mac is needed to *fill or refresh* the cache, not for every frame: once a folder is cached the frame plays from its own eMMC, so a Mac that then dozes off does not interrupt playback (the keep-showing-cached behavior above). The distinction owners care about is **display sleep vs system sleep**: the *monitor going dark* is display sleep and the Mac stays fully awake and serving (a non-issue); only a full *system* sleep pauses the server. macOS already exposes the fix as a user setting, **"Prevent automatic sleeping when the display is off"** (keeps the system awake with the monitor dark), with **"Wake for network access"** a weaker safety net (fine for occasional access, not continuous video). The zero-config, next-owner answer is for **the app to hold the same power assertion it already uses when the Mac is the display, tuned to keep the SYSTEM awake while letting the MONITOR sleep** (a small Mac-app-side polish, not in the frame checkpoints), so an owner never has to hunt for an Energy toggle. Net: monitor dark is fine; only a genuine full sleep interrupts an *uncached* fetch, which the assertion (or the setting) prevents.
- **Apple TV (the priority target; shipped in the tvOS app, and it needed no folder code of its own).** The uniform app-HTTP source made a **thin native tvOS app** straightforward: discover the Mac over Bonjour, pull the folder list and manifest over HTTP, play files natively (AVPlayer for video, an image view for stills), full-screen, with the same prefetch buffer and no persistent storage. It renders **file content only** (Folder Collections and uploaded images/videos); it **cannot** show the connected/generative HTML pieces (§8), because tvOS has no web engine (no WKWebView), so those remain a frame/Mac capability (an iPad, which has a web engine, could show them). The source is shared and uniform (the app's HTTP folder); only the renderer differs, the frame and Mac use the Chromium display page while the Apple TV re-expresses the rotation loop natively. This is the payoff that drove the reach decision: self-hosted art on any TV.
...
- **E30, the Host and the copy.** First built as a proxy (forward while online, replay the last answer
  offline), then narrowed the same day on Matt's rule that a Connected piece goes online once, at Add, and
  never after, the archive bundles included. As shipped: the Add step (`mirrorBundle`, for a `liveRpc`
  collection) makes the piece's one request itself, `primeLiveState`, and keeps the answer in
  `rpc-cache.json` beside the bundle, keyed by the request body's hash. The body is built to match the
  piece's own byte for byte (captured from the piece in a headless Chrome: an `eth_call` to its hook
  contract for the token number within the project, block 0, 36; the selector and contract read from the
  mirrored HTML, the captured values as fallback; hash checked equal to the captured request's). The
  route `POST /collections/<slug>[/<token>]/rpc` answers from that file only, never the network, and the
  collections CSP no longer opens the public node, so the piece's embedded fallback nodes are blocked too.
  `withConnectedFlags` hands the piece `rpc` as its node, relative to its own page, so the same value
  reaches the route on a Host and the copy scheme on the iPad. Pieces added before this existed (Matt's
  two tokens) are primed once by `primeMissingLiveState` at the first start with internet, so no re-add.
  The piece shows the network as it was the day it was added; a fresh look is a remove and re-add. The iOS scheme handler answers a POST to `…/rpc` from the bundle's cached answers by the
...
**Device-verified 2026-09-16** (Matt, Safari on his iPhone against the updated frame): inkField painted
live, the square pieces centered, and the black freeze did not recur, so its Checks row was dropped; it
returns as an item only if it is seen again.
...
Reported from the frame: the transition into this piece shows a black stage for a beat. Measured before touching anything, on a Mac, in the real player.

**The reveal was firing before the artwork existed.** The display starts its crossfade on the iframe's `load` event, which for a p5 sketch is the wrong moment by construction: p5 waits for that same event to build the sketch, then runs `preload()` and `setup()`. So the fade landed on an empty document and the stage sat black for **1.7 seconds** while the sketch caught up. Hiding p5's own `#p5_loading` screen (which we do, for a chrome-free stage, §6) is what made the gap read as black rather than as the artist's purple "loading…" text; the gap was always there. Fixed with the `awaitPaint` registry flag described in §7: reveal on the first painted frame, so the outgoing piece holds the stage. Display-side only, no re-add needed, and it generalizes to any bundle that loads assets before it paints.

**Almost none of the wait was network.** Every file transferred inside 0.3 seconds, warm or cold. The cost was **image decode**, and it was concentrated in the two easter-egg GIFs: 16 ms and 18 ms for the two photos, against **788 ms and 819 ms** for the GIFs, which are 2500² at 2 frames each, roughly 100 MB of pixel buffers. `preload()` blocks the first paint on all four, so with **Animate off (the default) the frame decoded 9.8 MB it then never drew**, on every pass through the rotation (a warm cache measured identically: decode does not cache). An `htmlReplace` pair now loads the two GIFs only under `?ooanim=1`, cutting the wait from about 1.6 s to about 0.9 s with Animate off and leaving Animate on untouched, still preloaded, so the piece is still revealed already animated rather than as a still that pops into motion. Keyed on the variable names, not the URLs, because `htmlReplace` runs before `localizeAbsolute` (the GIFs are still absolute Pinata URLs at that point, and localize still fetches them into the bundle for Animate on).

**One behavior traded, deliberately.** The artist's `touchEnded` tap-to-animate is guarded to match (`if (dayEaster && nightEaster)`), since with the GIFs unloaded a tap would draw undefined images. The frame has no pointer, so this is only reachable by clicking the piece on a computer display, where with Animate off it now does nothing instead of switching to the animation. Matt's call, on the standard that the displayed art must be unaltered while the code need not be.

This one is a **bundle** transform, unlike the reveal fix, so an existing copy keeps the old behavior until the piece is removed and re-added (§8: an add always re-mirrors). Re-adding also picks up the `#p5_loading` hide for any copy mirrored before that shipped.
...
### 2026-08-06: Export compliance answered declaratively, so no build is flagged again

Neither app set `ITSAppUsesNonExemptEncryption`, so App Store Connect flagged each uploaded build "Missing Compliance" and asked the App Encryption Documentation question by hand. Both apps use only the OS's HTTPS (no bundled or implemented cryptography), so `ITSAppUsesNonExemptEncryption = false` now sits in both `project.yml` files (`92a1181`), answering it declaratively and permanently. Confirmed on the 1.6.2 tvOS upload, which was not flagged. **Do not remove those keys**, or the dialog comes back on every upload. (Recorded in §17 as a to-do until 2026-08-08, when it moved here as shipped work.)

### 2026-08-06: Auto Display shipped (1.7.0), the Mac shows art after a period of inactivity

Shipped in **1.7.0** (`f38c025`) and polished in **1.7.2** (`8685759`). This entry is the build note; the full design record, every decision and the reasoning behind it, follows it below (moved here from §17 on 2026-08-08, since §17 is the list of what is still open).
...
**The design record (moved from §17, 2026-08-08).**
...
**It is a trigger, not a renderer.** This is the design's whole point. The Mac app's display is already a separate process, a Chrome kiosk pointed at the active Host's `/display` (`DisplayController.show(url:)` / `stop()`, and the 2026-07-01 real-Chrome decision in §20). Auto Display renders nothing of its own; it only decides *when* to call those two existing methods. So it inherits the exact fidelity of the normal display: every Connected Collection, Folder Collections, video, and the Sound setting all behave identically, with nothing reimplemented and nothing lost. `DisplayController` already takes a `ProcessInfo.beginActivity` power assertion while the kiosk runs, so the screen will not blank mid-piece; that part is done.

Detecting both the idle and the wake needs **no permissions**: poll `CGEventSource.secondsSinceLastEventType` (a global event monitor, which would need Accessibility, is not required for either direction). First-run friction stays at zero.

**Why not an actual screen saver.** Recorded so it is not re-derived. A macOS screen saver is a `.saver` plugin whose view Apple draws inside its own host process; it cannot host another application, so the Chrome kiosk could never live there. Apple built a modern screen saver API for its own aerials and keeps it private, leaving third parties on the legacy `legacyScreenSaver` host, which is a long-neglected area (memory leaks, shutdown bugs, an `isPreview` bug in macOS 26 with no known workaround). The obvious substitute, a `WKWebView` inside the saver, fails twice over: WebKit in that host has been broken since Sonoma (video does not play, `requestAnimationFrame` stalls, and content disappears after a few seconds), and WebKit is in any case the engine that tile-corrupts Golden Lining, which is precisely why the Mac app drives real Chrome. A real `.saver` would therefore be a second, worse renderer. Auto Display is an app-level idle trigger instead, which is both cheaper and higher fidelity.

**Multiple displays: cover them all, art on the main one, the rest black.** A real screen saver fills every attached screen, so covering only the main display would leave the others glowing with the owner's work while they are away, which defeats much of the point. But putting art on every screen is wrong here too: the rotation is client-side (`display.js` keeps its own `pos` and arms its own `setTimeout(advance, durationMs)` from each piece's reveal), so N kiosks would be N independent rotations that drift apart within minutes, and each one costs a full video decode plus any live p5/three.js sketch. Blacking the secondaries satisfies the convention for free and avoids both problems. "Art on every display" is a possible later option for a gallery wall, not the default.

**Reversed 2026-08-10 (Matt): art on every screen, as the default. Do not build from the paragraph above.** The argument is the same parity that shaped the setting itself. The macOS screen saver is a **single global choice applied to every attached display**, with nothing to configure per monitor; a feature that emulates it should cover every screen the same way. **No new setting is added**, because Auto Display already has exactly that shape.

The drift objection above is true as fact but was wrong as an obstacle, and it understates itself: in Shuffle the bag is client-side `Math.random()`, so two displays never agree even on the **first** piece. They start apart rather than drifting apart. **That is now the intended behavior**, and it is also what a screen saver does, since each display runs its own instance. Syncing screens to the same piece was weighed and **closed** (`ROADMAP.md` D17): it would mean making the server authoritative about what is showing, which collides head-on with the deliberate reveal-timed re-arm (§7), and it would mean changing `player/`, shared with the frame, for a Mac-only feature.

**Audio is deliberately not special-cased.** N screens with Sound on means N streams, which is simply what showing two things with audio means; Sound Off silences all of them at once, because one Mac has one Sound setting (§12). Not a defect, so no per-kiosk mute and no secondary-display URL flag.

**A screen configuration change ends the session.** Unplugging a display mid-session would otherwise leave macOS to migrate the orphaned full-screen kiosk onto the primary and stack it on the art already there, a state that cannot happen today because an orphaned blackout window is just an empty black rectangle. Rather than reconcile kiosks against `didChangeScreenParametersNotification` (which fires repeatedly through a single config change and needs per-screen identity tracking), observe it and call the existing `finish(stopDisplay: true)`. Plugging or unplugging a display is a human at the machine, the same signal as a keypress, and Auto Display already exits on that. The idle timer keeps ticking, so the art returns on its own after the interval with nothing for the owner to do. Note today's `AutoDisplay` has **no** screen-change observer at all, so a screen plugged in mid-session already shows an uncovered desktop; this closes that too.

**What the build has to do**, all of it inside `mac-app/` so the frame's playback loop is untouched: `DisplayController`'s single `kiosk` becomes an array (`show()` currently opens with `stop()`, "replace any running display"), with `stop()` and `reconcileIfExited()` iterating it. **One `--user-data-dir` per screen**, because Chrome is a singleton per profile: a second launch against the same profile silently joins the first and ignores every flag including `--kiosk`, which fails as a plain browser window rather than as an error. Position each kiosk with `--window-position`, **converting AppKit's bottom-left-origin global space to Chrome's top-left**; the two agree only at the primary screen's origin, which is why the shipped `pinToPrimaryScreen` gets away with a hardcoded `0,0` and never had to face this. `showBlackout()` is kept as the fallback for a screen whose kiosk fails to launch. The power assertion needs no change, being already guarded by `activity == nil`. Remaining cost is runtime, not code: N Chrome instances each running a full render, which is cheap for file art and N full generative renders for Connected pieces.

**The setting mirrors Apple's.** On macOS 26 this lives in **System Settings > Wallpaper**, in the Screen Saver options sheet, as **"Start Screen Saver"** (it is *not* in Lock Screen, and there is no Screen Saver pane in the sidebar; verified from Matt's Mac 2026-08-05). It is one pop-up menu with fixed presets and **Never** as the last item in the list rather than a separate checkbox. Auto Display copies that shape, including labelling the row with the feature's **name** rather than a description of it (Apple's row is "Start Screen Saver", not "Start the screen saver when inactive"), so the name Help uses is visible in the UI and the row sits consistently beside the noun-labelled Dock icon row: it reads **"Auto Display: After 10 minutes"**. It copies the **"After N minutes"** phrasing (note Apple uses "After" for the screen saver and "For" for the display-off settings), and mirrors the full interval list: **After 1, 2, 3, 5, 10, 20, 30 minutes; After 1 hour, 1 hour 30 minutes, 2 hours, 2 hours 30 minutes, 3 hours; Never.** Default **Never**, so the feature is opt-in.

Mirroring the whole list, long end included, follows Matt's stated principle: someone whose workflow already tolerates a 2-minute system screen saver should get the same choice here, and the same reasoning runs in both directions. Revisit only if the long intervals prove actively confusing.

**Auto Display must beat the display-off timer, and the setting should say so.** This is the one genuine footgun. "Turn display off when inactive" (System Settings > Lock Screen) is a separate macOS timer that Auto Display does not control; on Matt's Mac it is 10 minutes. If Auto Display's interval is longer, macOS blanks the screen *before* the art ever starts, and the art plays to a dark panel. Once the art is running our assertion holds the screen lit indefinitely, so this only decides the initial race. Because Apple's interval list runs to three hours, **most of the menu is longer than a typical display-off setting**, which makes this a required behavior rather than polish: read the system display-off value (the app is not sandboxed, so it can) and warn in the setting when the chosen interval cannot win. Without that, a majority of the options silently do nothing.
...
**Rotation order needs no special handling (resolved 2026-08-05).** Each trigger reloads `/display`, so a Sequence rotation always opens on its first piece. Rather than persist or randomize a starting index, this simply follows the owner's existing Rotation setting: a fresh kiosk gets a fresh shuffle bag, so **Shuffle genuinely opens on a different piece each time**, and Sequence starting at piece one is just what Sequence means, consistently on every display. Anyone who dislikes the repeat picks Shuffle. Matt's call, and it closes the item with no code.

**The Chrome cold-start gap is a non-issue (resolved 2026-08-05).** A trigger launches Chrome as a new process, so there is a second or two before art appears (longer if the first piece is a heavy connected one, which `display.js` allows up to 30 seconds to become ready versus 500ms for everything else). That delay lands at the exact moment nobody is watching, because the trigger condition *is* that the owner walked away. Pre-warming a hidden kiosk would fix an unobserved delay by decoding video around the clock, which is the wrong trade. Accept it.

**Device test: the macOS screen saver does NOT interrupt the art (verified 2026-08-05).** Matt set Start Screen Saver to "After 1 minute", opened the OpenObject display, and left the Mac untouched: after two minutes the rotation was still running and the system screen saver never appeared. So Auto Display does **not** require the owner to disable their screen saver, and the earlier assumption that it would (recorded here on 2026-08-02) is wrong. One residual worth knowing if this is ever reported failing: the suppression may come from our `beginActivity` assertion *or* from Chrome's own power-save blocker during video playback, and only the former would necessarily hold for a rotation of pure still images.
...
Broader lesson (recorded so it sticks): the connected-collection mirror bakes transforms that change the viewer experience (forced collector mode, `hideSelectors`, `htmlReplace`, black stage, and here an overlay suppression). Those are to be enumerated for the owner's sign-off, not decided unilaterally as art-vs-chrome; a process visualization (grids, construction lines, parameter readouts) can be intended art, so default to keeping it. Files: `player/src/collections.js` (`INKFIELD_HOOK` and its inkField comments), `docs/HANDOFF.md` (§8, the two entries below, this entry). No Setup Guide change (the owner-facing description is unaffected). (Matt, 2026-07-29.)

### 2026-07-29: inkField frame-safe pixel density (`framePixelDensity`; real-frame follow-up)

Testing the just-added inkField (entry below) on the real XXL: at the bundle's collector-mode `pixelDensity` 2 (double the recording's baked canvas) the weak GPU could not keep up. A single piece drew but stuttered; two pieces in a 90-second rotation overran it (lost the WebGL context, blanked, then flashed between the two paper grounds as it crossfaded). A single **Pin** was stable-but-slow. Fix: a per-collection **`framePixelDensity`** (inkField = 1) that a **frame** display applies via the bundle's own `?_pix:1`, rendering at a quarter of the pixels (CSS-upscaled to the panel: softer, but smooth, and fine for an ink painting), while a capable display (a Mac) keeps the sharper default 2. It is decided **at display time by device role**, not baked at add time (which answers the natural question, no, density need not be fixed when a piece is added, nor exposed as a user setting): the Host role (`frame` | `standalone`) now rides in `/api/display` alongside the items, so `display.js` reads it **race-free** (the role arrives before the first render, which matters for a Pin that renders once) and appends `_pix:1` only on a frame. Because it is a play-time URL param the mirrored bundle already honours, an existing frame needs only a **Software Update**, no remove/re-add.

Verified in the browser harness: `?_pix:1` yields `pixelDensity` 1 (a 900×900 canvas backing for the square token, vs 1800×1800 at the default, a quarter of the fragment work) and still renders clean (collector, HUD off, loop armed); and the display URL builder appends `_pix:1` on a `frame` role and nothing on `standalone` or for a collection without `framePixelDensity` (unit-checked). The full display path (`/api/display` role → `display.js`) was not run in the harness (it needs the full server, which raises a second mDNS responder), and on-frame smoothness is Matt's to confirm after updating. No Setup Guide change: the density is automatic and per-device, with no new owner-facing control.

Files: `player/src/collections.js` (`framePixelDensity` on inkField), `player/server.js` (`framePixelDensity` passthrough in `withConnectedFlags`, `role` in `/api/display`), `player/public/display.js` (frame-only `_pix` in `render()`, `deviceRole` from the poll), `docs/HANDOFF.md` (§8, this entry). (Matt, 2026-07-29.)

### 2026-07-29: Connected Collection added, Aluan Wang's *"inkField"* (Ethereum; first hash-seeded shared bundle)

The fourteenth Connected Collection (§8). inkField is a generative ink-painting series whose per-piece seed rides in the **URL hash** (`index.html#<tokenId>`), not a `?query` or a per-token path: every token shares one IPFS bundle and the p5.js sketch reads `location.hash`, so it is a **shared bundle** and the owner enters their Token ID (a true series; the Pendulum lesson). Two general seams were added. (1) **Hash forwarding in the display** (`display.js`): the connected iframe URL now carries the official URL's `#fragment` as well as its `?query` seed, split so the query never swallows the fragment (backward-compatible; no other collection has a hash, so the fragment is empty for them, and the per-token path is unchanged). (2) **A `{token}` placeholder in `extraAssets`** (`mirrorInto`), resolved from the sourceUrl hash, so the per-token recording `lib/<tokenId>.json` (which the sketch `fetch()`es by hash, invisible to the `src`/`href` scan) is mirrored; the font `lib/inconsolata.otf` is named outright (a failed `loadFont` in `preload()` stalls `setup()`). The bundle defaults to an in-page artist editor; a `htmlReplace` forces the artist's **collector** mode (clean UI, auto-play, loop pinned on). §6 stage cleanup: a `<style>` drops the dotted page pattern and the canvas border, and a new `stageHook: 'inkfield'` (`INKFIELD_HOOK`) forces the page background black (inline `!important`, since the artist paints it the paper colour, shown in a non-square token's letterbox). (An earlier version of this hook also suppressed the sketch's grid / brush-parameter overlay; that was reverted the same day, see the entry above: the overlay is the artist's intended render, not chrome.) Per-token format varies (#31 Portrait, #38 Square); the sketch sizes its own canvas and scales-to-fit on black, so no collection-level `aspect`. Self-animating, so no Animate control.

Verified in stages. The **render** was approved by Matt in a foreground browser. The rest was checked end-to-end **without** starting the full player, to avoid a second Bonjour/mDNS responder on the Mac that can disrupt `openobject.local` (see the 2026-07-12 stray-server lesson): a script drove the **real `mirrorBundle`** for #31 and #38 into a temp data dir (collector forced, `oo-stage` `<style>` and `INKFIELD_HOOK` injected, the gateway beacon stripped, `lib/<token>.json` plus the font mirrored, both recordings accumulating in the one shared dir); the mirrored bundle was then served under the **exact `/collections` CSP** and reached `setupComplete` with the HUD off and a black stage and **no CSP violations** (p5's two `eval()`s are its Friendly-Error sketch-linting inside `try/catch`, harmless under the no-`unsafe-eval` policy); and the `display.js` hash-forward URL builder passed a unit check across hash-only, query-only, mixed, and per-token shapes. **Not verified here:** the live paint and loop, because the preview tab is always `visibilityState: hidden`, which throttles `requestAnimationFrame` (the same limitation noted for The Bloom and Golden Lining); confirmed armed (`isLooping`, `loopToggle` 1) and it paints in a real foreground tab. **Frame GPU untested:** heavy WebGL at `pixelDensity` 2, smooth on the Mac, needs a real-frame check on the XXL.

Files: `player/src/collections.js` (registry entry, `INKFIELD_HOOK`, `{token}` in `extraAssets`, the hook selector), `player/public/display.js` (hash-fragment forwarding), `docs/HANDOFF.md` (§8, this entry), `docs/SETUP-GUIDE.md`. **Setup Guide updated** (inkField in the connected-pieces list). (Matt, 2026-07-29.)
...
`display.js` hard-set `el.muted = true` on every `<video>`. That is now driven by a new **web display Sound setting** (control panel, Rotation tab; stored `display_muted`, default off so audio plays). The subtlety is autoplay, not the toggle: Chrome refuses unmuted autoplay unless launched with `--autoplay-policy=no-user-gesture-required`, which both kiosks pass but a plain browser opened at `/display` (the README path) does not. So the video render attempts unmuted playback and, on a rejected `play()`, retries muted: art never stops, and at worst it is silent on that one un-flagged path. A live Sound flip folds into the piece already on screen (`syncMute()`), which matters for the case actually expected in use, a **pinned video** that never advances to pick the setting up on its own.

The setting rides `/api/settings` (validated boolean) and `/api/display` (all three source branches: library, local folder, remote folder), so every Display reads it same-origin with no new endpoint and no server-side per-client logic. **Sound is a master gate, not a video-only mute** (the reconciliation §10 of the tvOS plan flagged, settled with Matt): Sound Off also silences a scored connected piece, by forcing its audio control to the silent value. That is done display-side in `gatedControls` (`display.js`), not on the server (the display decides, per §11): the server keeps sending the raw collection controls plus the raw `muted` flag, and the display combines them. Today the one audio control is *The Bloom*'s `music`, forced to `off`, which reuses the existing `?oo_music` param the bundle already honors, so it needs no bundle change and works with pieces mirrored before this shipped; the piece then renders its own ambient/silent form. The gated value is folded into the connected `sig`, so a live Sound flip re-renders the piece (a crossfade-reload, the same one an Animate/Fit change triggers) while a connected piece with no audio control does not reload, and uploaded video still mutes instantly in place (`syncMute`). The per-collection **Music** control is the finer choice underneath, biting only when Sound is On (the collection control says whether a piece is scored at all; the display control says whether this screen makes noise; the display wins). Nothing to verify on the frame: it opens no audio output at all (2026-07-20 entry below), so its own kiosk stays silent (a connected piece already takes its ambient fallback there); the frame's Sound setting still governs a Mac rendering that frame's `/display`.

Verified in the browser harness: the setting round-trips through `/api/settings` and `/api/display` (an invalid value is rejected `400`); the control panel's Sound seg defaults **On**, toggles to Off, and reflects the saved state after a reload; a pinned test video renders with `el.muted` tracking the setting, and flipping the server setting muted the **same** on-screen node without a re-render (`syncMute`, confirmed with a tag that survives only if the node is not re-created). The **master gate** was checked the same way: with *The Bloom* pinned, its iframe `?oo_music` reads `on` at Sound On and `off` at Sound Off, each flip re-renders the piece to a fresh node in both directions, and the Bouncing OpenObject Logo (controls but no audio) keeps its node and gains no `oo_music` on a flip. **Not verified in the harness:** audible playback and the retry-on-rejection path, because the preview tab is always backgrounded (the video never leaves `paused`, and `play()` neither starts nor rejects); those need a real foreground Mac, per the plan. Files: `player/public/display.js` (conditional mute, retry-muted fallback, `syncMute`, `gatedControls` master gate, header), `player/server.js` (`display_muted` in `currentSettings` / `PUT /api/settings` / `/api/display`), `player/public/control.html` (Sound seg), `player/public/control.js` (seg wiring), `docs/HANDOFF.md` (§6, §12, this entry), `docs/SETUP-GUIDE.md` (Sound section), `CLAUDE.md`. **Setup Guide updated** (the new Sound setting). (Matt, 2026-07-24.)

### 2026-07-24: Idle-splash address prefers a real LAN interface (tvOS plan Phase A)

Pre-existing bug, surfaced by the tvOS plan (§14) but worth fixing on its own. The display's idle screen shows an "add art at ..." hint whose address is `addresses[0]` from `lanAddresses()` (`player/server.js`, via `/api/system`, consumed in `display.js`). That was raw OS enumeration order, not reachability, so on a multi-homed machine it could surface a VPN tunnel, a virtualization bridge, or a link-local (no-DHCP) address the viewer cannot reach. VPN tunnels and virtualization bridges qualify for the list, and Matt's Mac is routinely dual-homed.

`lanAddresses()` now skips interfaces that are never the LAN route (VPN / virtual / overlay name prefixes: `utun`, `wg`, `tailscale`, `docker`, `bridge`, `vmnet`, `veth`, `virbr`, and similar) and link-local `169.254.*` addresses, then orders real private-LAN (RFC1918) addresses first: a dependency-free proxy for the default route, with no platform-specific route lookup and nothing shelled out. The control panel's "reach me from your phone" list improves the same way (it already showed the whole list; now the list is real LAN addresses, best first). **No UI change.**

Verified: on this dual-homed Mac both real `en0`/`en1` LAN addresses are kept, best-first; a synthetic interface set (a Tailscale `utun`, a `docker0` bridge, a `169.254` link-local, a public address on a kept interface, and a real `192.168` address enumerated last) drops the first three and ranks the private address first. Files: `player/server.js` (`lanAddresses` rewrite), this entry. No Setup Guide change (behavior only; the shown address is now more correct). (Matt, 2026-07-24.)

### 2026-07-20: Layer cleanup no longer deletes the piece it is waiting on (blank stage on a lone piece)

Changing a setting on a **lone or pinned** piece could black out the display permanently. Found while testing the Music control, but pre-existing and unrelated to audio: **Fit/Fill, Animate, and the speed slider reach it too.**

`freeHiddenLayer()` exists to stop a previous piece's iframe animating invisibly behind the current one, which is what fixed the early complex-piece slowdowns, so its behaviour is load-bearing. It picked its target with two tests: the layer is `layers[1 - front]`, and it has no `.show`. Both describe a **stale** layer once a crossfade has settled. But they equally describe a layer with a render still **in flight**: `front` does not flip until `reveal`, and the incoming layer gets `.show` only at `reveal`. The `lastShowAt < 700` guard did not save it either, since `lastShowAt` is set inside `reveal` and so still held the previous piece's timestamp.

It was not a race. In `apply()`, `show()` fills the back layer and `freeHiddenLayer()` runs **two lines later** in the same tick, whenever `items.length === 1`. The new iframe was destroyed immediately, its `load` therefore never fired, and the 30 s reveal backstop then faded in an empty layer: the outgoing piece held for up to 30 s, then black until something else re-rendered. Only the lone/pinned path was affected, because that is the only place `apply()` calls the cleanup; with two or more pieces it is reached only from the safe `setTimeout(freeHiddenLayer, 750)` after a reveal.

Fix: track the layer holding an unrevealed render (`pendingLayer`, set in `show()`, cleared in `reveal`) and never free it. Three lines. Deliberately not just moving `lastShowAt` earlier, which would buy 700 ms when a heavy piece on the frame can take longer. **The cleanup itself is unchanged**, so nothing stays alive longer than before.

Verified by A/B on the real display with only that guard toggled, flipping Fit on a lone piece: disabled gave both layers empty with zero iframes (`fit-fill` applied, proving the render had happened and been deleted); enabled gave the piece on screen with the outgoing layer still correctly freed, confirming the performance cleanup still runs. No user-facing behaviour changes, so no Setup Guide change: this is a malfunction, not a documented behaviour. Files: `player/public/display.js`, this entry. (Matt, 2026-07-20.)

### 2026-07-20: Audio un-dropped for "The Bloom" via a Music control (§8, §12)

OpenObject muted every piece from the start ("silent art on a wall", §12), a rule written when the speakerless XXL frame was the only display. With the Mac now the primary display that rule was reversed, and this change applies the reversal to the one collection it actually cost: Chaz Wesley's *The Bloom*, the only Connected piece with a soundtrack. Scope was deliberately held to that piece; **uploaded video is untouched and still muted** at the element in `display.js`.

The piece now carries a **Music** control (`controls`, default **On**). Three findings shaped the work. (1) **The mute was one line, not the architecture.** `BLOOM_HOOK` no-op'd `HTMLMediaElement.play` before firing `startExperience()`; that override is now conditional on `?oo_music=off`, and the off path is the unchanged prior behavior. The surrounding stack needed nothing: both kiosks already launch Chrome with `--autoplay-policy=no-user-gesture-required`, and the collections CSP already allowed `media-src 'self'`. (2) **The audio was never in the mirror.** `CONFIG.MUSIC_URL: 'TheBloom.mp3'` is a bare string in a JS config object, and the asset scan only matches `src=`/`href=` attributes, so every bundle since the piece was added omitted 2.3 MB of a 2.6 MB artifact. A new **`extraAssets`** registry seam names such files explicitly and folds them into the existing fetch loop (the additive counterpart to `dropScripts`). Existing installs must remove and re-add the piece to pick the track up, since that is what re-mirrors. (3) **The music is not decoration.** The sketch runs an FFT over the track and derives bass/mid/high energy that drive the sunset gradient and the flowers' motion, falling back to a canned ambient sine when nothing plays. So Music On restores a music-reactive artwork rather than merely adding sound, which is why it defaults On. Serving the mp3 same-origin is also what lets `createMediaElementSource` read it untainted, so the analyser connects.

`dropScripts: ['p5.sound.min.js']` **stays**: it was always a renderer-hang fix, not a mute, and no audio path in this piece goes through p5.sound. A two-option `select` was used rather than a new boolean control type, matching the Chromie Squiggle's binary Background dropdown and needing no new plumbing.

Testing then turned up the artwork's **own music panel**, which an earlier note here had wrongly called unreachable: clicking the artist's signature toggles it open, exposing Play/Pause/Mute and an "Upload MP3 or WAV" picker on the stage. That is chrome an art display should not have (§6), and inviting a viewer to swap the score is worse, so `#settingsPanel` joined `hideSelectors`. Hiding it removes the only in-page way to start audio where autoplay is refused, so the hook gained **click to start the music** (see §8). The two changes are a pair: hiding the panel alone would leave the documented "run `npm start`, open `/display` in a browser" path permanently silent, since that surface gets no kiosk autoplay flag.

Verified locally against the real IPFS bundle: the mp3 mirrors at its exact 2,298,660 bytes, the player serves it (`206`), and the artwork's own console reports `Audio ready to play` then `Audio playing` then `Audio analyser connected`, with `p5.sound` still absent and `#startOverlay` still hidden. Server-side validation rejects an unknown value. **Verified on the real XXL** (2026-07-20): the frame pulled the change over Software Update, a remove-and-re-add mirrored the track (audible by fetching `/collections/chazstract/TheBloom.mp3` straight off the frame in a browser), and the piece displays with no slowdown from the extra 2.3 MB. The frame is nonetheless **silent and non-reactive**: it has an `HDA-Intel` card, but with Music On, the piece pinned and `openobject-kiosk` freshly restarted, all four playback substreams read `closed`, so Chromium opens no output and the sketch takes its ambient fallback. The control is therefore inert on the frame, which matches the eyeball result (no discernible difference between On and Off there). Left alone deliberately: adding PulseAudio/PipeWire to the appliance image to win a subtle motion difference on a speakerless panel is a bad trade against keeping that image minimal. **Not verified in the harness:** the on-screen result and the live in-place toggle, because the preview tab is always `visibilityState: hidden`, which suspends `requestAnimationFrame` (canvas art never paints) and throttles the 5 s poll. Those need a real display. Testing a lone pinned piece also surfaced a pre-existing blank-stage bug in `freeHiddenLayer()`, first described here as an unreproducible race. It is neither: see the 2026-07-20 layer-cleanup entry above, where it is diagnosed as deterministic and fixed.

Files: `player/src/collections.js` (registry entry, `extraAssets` in `mirrorInto`, `BLOOM_HOOK`), `docs/HANDOFF.md` (§8, §12, this entry), `docs/SETUP-GUIDE.md`. **Setup Guide updated** (the Music setting and the remove/re-add note). (Matt, 2026-07-20.)

### 2026-07-13: Version always visible in About + tab title self-identifies (Frame/Local)

Two small control-panel legibility fixes. **Tab title.** `control.html` serves one static `<title>` ("OpenObject · Control Panel") to both the frame and any standalone Host, so two open panels (the frame's plus this computer's, the setup an owner uses) read identically and can't be told apart in the browser. `loadSystem()` now stamps the title from the device role `/api/system` already returns: the frame reads **"OpenObject · Control Panel (Frame)"**, any other Host **"(Local)"**. It is role-based, not URL-based, so a tab reads the same however it was reached (localhost / openobject.local / LAN IP). "Local" over "Mac" because a standalone Host is not necessarily a Mac (a plain `npm start` on any computer is standalone too); "Frame" is exact (`role === 'frame'`). **Version in About.** The running version only appeared in the Software Update card's "Current version" line, and that whole card is hidden on a non-git Host (the Mac app updates via Sparkle, not git-pull, per the 2026-07-02 device-aware entry), so the web control panel showed the version **nowhere** there. Added a plain **"Version X.Y.Z"** line to the About card, populated from a new `version` field on `/api/system` (the same `package.json` version the update payload uses). Just the number, no date or commit: the commit fingerprint stays in the Software Update card on the frame, where the precise "did it update" signal belongs. The Mac app's native About keeps showing "X.Y.Z (build)" the macOS-standard way (`CFBundleShortVersionString` + `CFBundleVersion`); that build number is an app-bundle counter with no meaning to the player/frame, so it is deliberately not carried into the web line. The About tagline is kept (a one-line descriptor is standard in an About and orients a next owner); version sits under it, above the links. Files: `player/server.js` (`/api/system` `version`), `player/public/control.html` (About version line), `player/public/control.css` (`.about-version`), `player/public/control.js` (title + version in `loadSystem()`), `docs/SETUP-GUIDE.md` (About lists the version), this entry. **Setup Guide updated** (About card now lists the version, findable whether or not the Software Update card is present). (Matt, 2026-07-13.)
...
Added a **Help** card to the Settings tab, folded by default like the other reference cards, placed after Network. Its body is device-aware (the same split as the Folder Collections card): on the frame it explains how to display a folder that lives on a Mac (host the Mac, add the folder on the Mac's Control Panel, then select it here in Rotation -> Source, and what "Mac unreachable" means); on a Mac Host it explains defining a folder here and, as a footnote, showing one on a separate OpenObject frame (such as an Infinite Objects XXL). Wording is casual and jargon-free with the actual UI labels bolded; it teaches the two-panel model that tripped an owner up live (define on the Mac while Hosting, select on the frame), which was confusing enough in practice to warrant in-product guidance. Same pass: the **Power** card is now collapsible and folded by default (wireCollapse), matching the other Settings cards, since it is rarely touched; its existing device gating is unchanged (the whole card still hides unless the Host can OS-power or offers a supervised restart, and Reboot / Shut down stay frame-only). Verified end to end locally: both device variants, the collapse defaults, and the Power gating with OO_SUPERVISED. **Files:** `player/public/control.html`, `player/public/control.js`, `player/public/control.css`; docs §17 and the Setup Guide. (Matt, 2026-07-12.)

### 2026-07-12: Post-outage robustness (folder dead-end escape, collision guards, honest Viewer buttons)

Fallout fixes after an evening where `openobject.local` stopped resolving on Matt's Mac. The root cause was NOT the Mac app: two stray Node servers (an orphaned `npm start` and a leftover dev/preview server) were each running a `bonjour-service` mDNS responder, and more than one responder on one machine contends with macOS's mDNSResponder and breaks `.local` resolution system-wide, even in Safari. Killing the strays plus `sudo killall -HUP mDNSResponder` restored it, and a clean-Mac reproduction confirmed the app's own Host mode coexists fine with the frame (steady HTTP 200 while hosting). Three fixes came out of it.

1. **Folder Collections dead-end (frame control panel).** When the frame's source is a remote (Mac) folder whose Mac is unreachable AND the frame has no last-known manifest for it (for example right after a frame reboot wipes the in-memory cache), `renderSource` showed "Library" in the Source box while the real source was the stale `remote:` ref, plus a bare "Mac unreachable" line, and re-picking Library fired no change event, so there was no way out. Added a "Switch to Library" button to every "Mac unreachable" panel (both error states) that PUTs `displaySource: 'library'`. Verified end to end locally. Files: `player/public/control.js`, `player/public/control.css`.

2. **Engine collision guards (player).** A clear EADDRINUSE message plus clean exit when the port is already taken (instead of an uncaught stack trace), and a best-effort, machine-global instance CHECK (`src/instance-lock.js`) that logs a heads-up when a second OpenObject server already appears to be running on the same machine. Both FAIL-OPEN: they never block startup or change advertising, since the frame is single-instance and must always come up discoverable. The EADDRINUSE path fires before the listen callback, so it never advertises. Files: `player/server.js`, `player/src/instance-lock.js`.

3. **Honest Viewer buttons (Mac app).** In Viewer mode, Open Control Panel and Open Display stayed lit even when the chosen Host had dropped out of discovery, and clicking did nothing (the action cannot resolve a Host that is no longer listed). They now disable, with a "Not reachable right now" note, when the Host is not currently discovered. Also a pre-flight in the engine host: before spawning, if something already answers `/healthz` on the port, it reports "Another OpenObject server is already running on this Mac" instead of a vaguer failure. Files: `mac-app/Sources/ContentView.swift`, `mac-app/Sources/EngineHost.swift`.

Player fixes reach the frame via self-update; the Mac-app fixes ship on the next signed build. Kept out of the Setup Guide: these are error-recovery affordances and internal guards, not user-facing flow changes. (Matt, 2026-07-12.)

### 2026-07-12: Tiles tiled-mode, frame perf fix then a user Quality control (§8)

On the real XXL the Tiled shuffle stuttered into a multi-second crawl instead of snapping (filmed on Matt's frame): tiles sat half-jumbled and only crept into place, though the cycle did complete. Cause: `TILES_HOOK` forced `imageSmoothingQuality: 'high'` every frame, so the p5 draw loop HQ-downscaled the ~3628² source across all 36 tiles on each paint. That is free on a desktop GPU (where it was built and "verified end to end") but far too heavy for the frame's GPU, which collapsed the frame rate; and because the sketch lerps per-frame, a low frame rate stretches each ~0.5 s snap into 10 to 15 s of stutter. First fix: run the animation at p5's default `'low'` resampling (bilinear, still smooth, not pixelated); Static (a native `<img>`) is unaffected. **Confirmed perfect on the frame.** Faithfulness verified too: token 86's actual sketch (pulled from the contract to IPFS) contains no `noSmooth`, no `imageSmoothing`, no `smooth()`, so it runs at the p5 default (`enabled=true`, quality `'low'`), exactly what the hook now sets; `'low'` is the artist's own browser render, not a downgrade (we had only ever *added* `'high'`). Then, rather than hard-pin one quality, the single **Display** choice was widened to **Static / Tiled (High) / Tiled (Medium) / Tiled (Low)**, mapping the chosen value to `imageSmoothingQuality` high|medium|low so an owner matches render cost to their display (High/bicubic for a Mac or other strong box, Low/bilinear for the frame, Medium/mipmap between). Pure reuse of the shipped `choice` seam: nothing outside the registry hardcodes the values (the server validates against the declared options, `control.js` renders them generically, `display.js` passes `?oochoice` through), so only `collections.js` changed. **On-frame test (Matt): Medium still stutters on the frame, so the shipped default is `Tiled (Low)`** (the frame-safe, artist-default quality; the mission device plays it smoothly out of the box). A capable display can raise it to High for a sharper downscale, and Medium stays as a middle option for hardware between the two. Deployed via `origin/main`; the frame re-mirrors on remove + re-add of token 86. **Files:** `player/src/collections.js`, `docs/HANDOFF.md`, `docs/SETUP-GUIDE.md`. (Matt, 2026-07-12.)

### 2026-07-12: Connected collection "Tiles" (Rich Caldwell) added (§6, §8)

Added the thirteenth Connected Collection, Rich Caldwell's *"Tiles"* (Ethereum `0x0a44d8e8386c261caf1738d038994f1c65f58dc7`): a per-token series where each token slices one ~3628² aerial photo into a 6×6 grid that Fisher-Yates shuffles on the artist's click. It introduces two reusable mirror seams and one new control shape. (1) A tolerant per-collection **`htmlReplace`** list (string or RegExp finds, applied before the asset scans) rewrites the artist's own source: because the tokens vary token to token (margin value, background colour, per-tile frame), the margin is regex-set to 0 (edge to edge, §6), the stage is repainted pure black, and the unused `p5.sound` script and mediabunny `importmap` are dropped, so the mirror localizes no dead weight. (2) A **`staticFromLoadImage`** step saves the sketch's absolute-URL source photo once as a fixed `oo-static.jpg` and repoints `loadImage()` at it, so one local file feeds both the sketch and a still view. (3) The single **Display: Tiled/Static** control reuses the existing `choice` / `?oochoice` plumbing but is the first where the choice picks *what renders*: Static shows `oo-static.jpg` full-bleed and stops the sketch; Tiled runs the sketch, forces `imageSmoothingQuality: 'high'` (p5 defaults to `'low'`, a measurably softer downscale of the 3628² source, confirmed ~2x less fine-detail sharpness), and drives a hands-free shuffle cycle (assembled ~7 s, then 4 shuffles ~6 s apart, reassemble, loop), since a frame can't click. Default Tiled. All display / control-panel / DB / server plumbing was reused unchanged; only `collections.js` and the docs changed. **Verified end to end** on the isolated preview build: token 86 added through the real add flow, the mirrored bundle is fully offline (`index.html` + local `p5.min.js` + `oo-static.jpg`, zero external refs), both modes render edge to edge on pure black, and the Display choice persists through the API both ways. **Files:** `player/src/collections.js`; docs §6, §8, and the Setup Guide. (Matt, 2026-07-12.)

### 2026-07-11: Folder Collections Phase B (frame) built and verified on the real XXL (§17)

Shipped the frame half of Folder Collections (§17): the XXL now displays a folder living on the owner's Mac, over the LAN, **verified end to end on the real hardware** (discovery, playback, the ephemeral cache filling, an unreachable Mac, and recovery). Built in checkpoints, all gated on `OO_ROLE=frame` so a standalone Mac is untouched: a device-role signal plus Bonjour **browse** (the client side of the existing advertise); the Mac serving its folder list and per-folder item manifests over an open, path-free `/api/shared-folders` (+ `/:id/items`); a remote-folder model (`src/remote-folders.js`) resolving a `remote:<host>:<folder>` ref to a live LAN address and manifest; an **ephemeral session cache** (`src/folder-cache.js`, ~20 GB cap plus a 15 GB free-space floor, wiped on reboot, on leaving folder mode, and by a manual Clear) with a prefetch worker; and the frame's cache meter plus Clear control, with `OO_ROLE=frame` set in the systemd unit. Four things changed from the 2026-07-10 design during the build (see §17 "Built (Phase B, frame)"): **order follows the Rotation tab** (a folder plays in the frame's Sequence/Shuffle, as it already did the duration; the per-folder Order control is gone); the frame **auto-discovers** a just-shared folder (it keeps polling, with a brief "Looking for your Mac…" while it searches); a frame that finds a Mac too old to serve folders shows **"A newer version of OpenObject is available for your Mac. Please update the app."** (a general version check against a `MIN_MAC_VERSION` constant, deliberately not the frame's own version, which rides main ahead of every release); and an unreachable folder **stays greyed with its name** ("Mac unreachable…") from a last-known manifest, playing on from cache. A separate app-wide fix landed alongside: **Shuffle now completes a full pass before repeating** (the display had been re-shuffling its bag on every ~5s poll, degrading Shuffle to independent random). The Setup Guide gained the frame specifics and the Mac-serving requirements in the same change (§16). Remaining (§17): the Apple TV client, a Mac stay-awake power assertion, and an in-app Help section. **Files:** `player/server.js`, `player/src/{identity,discovery,remote-folders,folder-cache}.js`, `player/public/{control.html,control.js,control.css,display.js}`, `installer/systemd/openobject-player.service`; docs §17 and the Setup Guide. (Matt, 2026-07-11.)

### 2026-07-10: Folder Collections Phase B design settled (the app serves every screen; §17)

Locked the Phase B design for **Folder Collections** (the frame, and as a forward client an **Apple TV**) on one stated priority: **the non-XXL experience must be easy and slick, above all the Apple TV; the XXL owner is the edge case and must not complicate the UI.** The load-bearing decision: **the OpenObject app on the Mac is the one server for every screen.** Because tvOS cannot read a file share (only HTTP), feeding an Apple TV requires the app to serve over HTTP anyway, so the frame is served the **same** way rather than via a second path. A mounted **SMB** share (the "no app for the frame" route) was explored across several turns and **dropped**: guest SMB is a fading, off-by-default macOS setting (a poor next-owner bet) and a stored login is a trust cost for a small proprietary app; **app-HTTP holds no credential and does not rot.** Accepted consequence: a Folder Collection **only plays while the Mac and its app are alive** (a reboot or quit stops it), fine for an occasional, left-running feature. This **resolves the picker question by removing it**: a folder is defined once in the **app's** control panel, where the Mac's **native folder dialog** is the only picker; the **frame never picks a folder**, its Rotation **Source** dropdown just **selects** from the folders the app serves (the Phase A frame-side "Add folder," which could only browse the frame's own empty disk, is removed). The frame cache is redesigned from the earlier persistent hard-capped mirror to an **ephemeral session buffer**: prefetch-and-accumulate, capped (**~20 GB default plus a free-space floor**), and **wiped on reboot, on leaving folder mode, and by a manual Clear**, so nothing persists at rest, the Library is never crowded, and the eMMC is spared over long runs (this retires the aggregate-cap / LRU / discard-flag / age-out machinery once weighed in §17). Two error states are specified (no Mac found when adding or selecting; a live folder going unreachable) so there are no dead clicks. The Apple TV is a thin native tvOS client (Bonjour + HTTP + AVPlayer/image views), **file content only** (no connected/HTML art, since tvOS has no web engine). One home, many screens: defined once on the Mac, selected on any. **Nothing built yet**; §17 rewritten to this design; the Setup Guide stays untouched until Phase B ships (§16). (Matt, 2026-07-10.)
