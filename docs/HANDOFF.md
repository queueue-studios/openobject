...
- **Noted harmless:** the "local file changes are present" line on the Software Update card is only `player/package-lock.json`, which `npm install` rewrote at provisioning; a fast-forward that does not touch it still applies (verified). To be quieted by moving the installer and updater from `npm install` to `npm ci`. (Matt, 2026-06-15.)
