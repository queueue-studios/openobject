...
### 2026-06-30: Display holds the screen awake (Screen Wake Lock); version bumped to 1.0.1
A device running the rotation should not dim or sleep mid-art. The display page now holds a **Screen Wake Lock** (`navigator.wakeLock.request('screen')`) whenever it is visible, re-acquiring it on `visibilitychange` after the system releases it (the lock drops on a tab switch or a manual lock). No setting and no UI: staying lit is the display's whole job, so it is always-on for `/display` (and only `/display`). Two honest limits, both handled by a graceful no-op: (1) the API needs a **secure context** (https or `localhost`), so it engages on the localhost display paths, the frame's kiosk (`http://localhost`) and "your Mac as the display" (`http://localhost`), but does nothing when the page is opened over `http://openobject.local` from a phone or another machine (not a secure context, so `navigator.wakeLock` is absent there); (2) the dedicated frame already stays on at the OS level, so this is belt-and-suspenders on the frame. **Verified on Mac** (localhost preview): secure context true, the API is present, and the request path runs and is caught cleanly; the actual lock grant could not be exercised headless (the automation context has no focused display, so the browser returns `NotAllowedError`), so the live grant is confirmed on a real focused screen. **`player/package.json` bumped 1.0.0 → 1.0.1** in the same change, so the panel's version line moves on the next update (it had stayed `1.0.0` while commits advanced past the v1.0.0 tag, which read as "did it even update?"); this is a patch bump (fixes + this small feature), tags/Releases unchanged (§20 2026-06-30 channel entry). **Files.** `player/public/display.js` (the wake-lock block), `player/package.json` (version). **Setup Guide:** the Mac display guide gains a one-line note that the Mac stays awake while displaying; the XXL Setup Guide is unaffected (the frame's OS already handles it).
