...
| ID | Item | Design record | Notes |
|----|------|---------------|-------|
| E12 | Auto Display shows art on **every** attached screen. Today it shows art on the main screen and blacks out the rest (`AutoDisplay.showBlackout()`), which is what changes. **Reversed 2026-08-10** (Matt): this is the **default**, not an option, because Auto Display exists to emulate the macOS screen saver, and the macOS screen saver is one global choice applied to every screen with nothing to configure per monitor. Auto Display already has exactly that shape, so no new setting is added | HANDOFF §20 2026-08-06 Auto Display record; §17 | Medium, and **Mac-app-only**: `DisplayController` holds one `kiosk` and `show()` opens with `stop()`, so it becomes an array; screen targeting already exists as `--window-position` (used by `pinToPrimaryScreen`). **Gotcha:** Chrome is a singleton per `--user-data-dir`, so a second launch on the same profile joins the first and ignores `--kiosk`; each screen needs its own profile dir. Each screen runs its own independent rotation (D17). Audio is **not** special-cased: N screens with sound on means N streams, which is expected, and Sound Off is the answer (Matt, 2026-08-10). Remaining real cost is N Connected renders; `showBlackout()` stays as the fallback for a screen whose kiosk fails |
