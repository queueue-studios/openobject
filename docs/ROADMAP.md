...
| E31 | Website | After Apple approves iOS 1.10.0: the landing page's Connected Collections line still says "Not supported on the Apple TV and iPad displays"; change it to the Apple TV only, on the network and from the local copy for the iPad and iPhone, and republish gh-pages (the site describes what is shipped, §15). Not before the approval | HANDOFF §15 "Cutting a release", §20 2026-09-17 | Tiny. One sentence in `site/index.html` plus the proven republish recipe |
...
| ID | Area | Item | Notes |
|----|------|------|-------|
| C1 | Network | Matt's iPhone never lists the frame by name: Safari on it cannot open `openobject.local` either (2026-09-02, 2026-09-15, 2026-09-17, Wi-Fi toggled), while the Mac and the iPad see the frame's Bonjour advert on every interface and the frame answers the phone by address. So the phone's Wi-Fi path is not carrying multicast DNS, most likely the router treating that band or client differently, not the app. Check: join the phone to the other band, or compare the router's multicast/IGMP and client-isolation settings per band; the app's typed address covers it meanwhile | Environmental, outside the app; recorded so it is not re-diagnosed as an app fault |
