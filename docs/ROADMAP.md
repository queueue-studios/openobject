...
| iPad + iPhone | **E3, the local copy, on a real device.** With the Host live, leave the app on the art until a tap shows **Local copy ready**; turn off Wi-Fi and relaunch: art plays at once, no Connecting; tap Hosts: the picker shows the Host's row tagged **Local copy** under the waiting line, the state the simulator could not show (Matt's frame stayed discoverable throughout). Also worth a glance: the overlay clears the iPhone's Dynamic Island, the fix found in the simulator pass | this change |
