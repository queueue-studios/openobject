...
|----|------|------|---------------|-------|
| E3 | Apps (iPad) | Offline / portable playback: the **iPad** keeps a **local copy** of the remembered Host's rotation and plays it with no network. **Committed 2026-08-10** (Matt); **design settled and built 2026-09-15** (simulator-verified, then **device-verified on Matt's iPad via TestFlight iOS 1.10.0 (3)** the same day, HANDOFF §20; ships with the 1.10.0 iOS App Store release): always on, no switch; a Host-named picker row tagged "Local copy" plus one status line in the stage overlay are the whole UI. iPad only, permanently: the Apple TV variant is closed by tvOS storage limits (D14) | HANDOFF §17 "Offline / portable playback" (the settled design block); `TVOS-APP-PLAN.md` §9 | Medium. A non-purgeable store in Application Support plus the persisted manifest, reconcile-on-poll with a grace period, the free-space reserve (one tenth of the disk, 5 GB floor, no fixed cap), the picker row, the four overlay states, the open-straight-to-the-copy launch path, the Demo Gallery never saved, and the Setup Guide's iPad section in the same change (§16). Press-and-hold removal deferred out of the first build |
| E26 | Apps (iPad) | Offline rotation controls on the local copy: **Order** (Sequence / Shuffle) and **Every** (the duration), shown ONLY while the iPad is playing its local copy, reached by tapping the "Playing local copy" capsule. An offline change lasts until the iPad sees the frame again. **Committed, built, and device-verified 2026-09-16** (Matt, TestFlight iOS 1.10.0 build 4, HANDOFF §20; rides with E3 in the iOS release) | HANDOFF §17 "Offline / portable playback", the "Offline rotation controls" bullet | Small. A persisted override applied when seeding or when the Host stops answering, cleared on the next successful poll; the engine already folds a duration or mode change in without restarting. Sound stays where it is (the picker's speaker) |
...
| E24 | Apps (tvOS) | Connect gives no feedback while it probes: the button stays enabled, nothing spins, and `URLSession.shared`'s default **60 second** timeout means it can look dead for a minute. Disable it while in flight, show progress, and use a short timeout. **Fixed in the iOS app 2026-09-16** (HANDOFF §20; rides in iOS 1.10.0 build 5). **Open: the tvOS half**, the same lines in tv-app's AppModel and picker, with the next tvOS release | HANDOFF §17 "Connect has no pending state" | Small. Found on a real iPhone 2026-09-02: a typed address appeared to do nothing, then connected on its own well after the tap |
| E25 | Apps (tvOS) | No way to retry discovery from the picker. The Bonjour browse only restarts when the app returns to the foreground with an empty list, so a browse that goes stale while the owner is watching the screen has no visible recovery. **Fixed in the iOS app 2026-09-16** (HANDOFF §20): the picker re-browses itself every 15 s while its list is empty, no UI. **Open: the tvOS half** | HANDOFF §17 "Connect has no pending state" | Small. Same 2026-09-02 session: toggling iOS Local Network permission killed the in-flight browse, and staying in the app meant nothing ever re-browsed |
| E27 | Player (frame, Mac) | **Black frame before a late-painting Connected piece on the frame** (Matt, 2026-09-16, before and after inkField). The display reveals a piece on its iframe's `load` event, which for a p5 sketch fires before anything is drawn; only `awaitPaint` collections wait for a painted frame, and that wait reads the page's global `frameCount`, which inkField keeps at zero while its sketch instance runs. Port the iOS layer's version back to `display.js`: wait for the first painted frame on every Connected piece, reading `p5.instance.frameCount` (a stopped one-frame sketch counts as painted), capped as today. Reaches the frame by Software Update | HANDOFF §20 2026-09-16, the device round | Small. Found on the iPad first, fixed there the same evening; the frame has the same fault |
| E1 | Apps (iPad) | Connected Collections on the iPad/iPhone app via `WKWebView`. **Phase one built 2026-09-16** (simulator-verified on the iPad against a copy of the Mac's library, HANDOFF §20): Connected pieces play while the Host is reachable, the web view at the Host's own mirror with `display.js`'s URL contract; offline the local copy skips them and plays the rest. **Device-verified 2026-09-16/17** on Matt's iPad and iPhone over the cable (HANDOFF §20: two clean passes, about 5 GB of headroom, no process death; Azulejo 25 s to ready, the rest under 4 s). **Next: TestFlight iOS 1.10.0 build 5**, then **phase two:** the copy carries each piece's bundle, served to the web view by an in-app scheme handler; the manifest already carries the URL fields. The skip mechanism and the "Chrome Only" pill stay unbuilt unless a piece actually fails (§17). The website's Connected line (`site/index.html`) changes with the iOS release that carries this | HANDOFF §17 "Connected Collections on the viewer apps" | **Medium** for phase two. Phase one's as-built record is the §20 entry of 2026-09-16 |
...
|----|------|------|---------------|
| E28 | Player (frame, Mac) | The outgoing piece freezes while the next Connected piece generates: same-origin iframes share the page's main thread, so a heavy synchronous generate stalls the sketch on screen. The iOS app now gives each piece its own web process. Chromium isolates only cross-origin iframes, so the frame's equivalent would be serving the incoming bundle from a second origin (another port, or a host alias), a design question with a same-origin-mirror cost to weigh (sessionStorage flags, RPC) | HANDOFF §20 2026-09-16, the device round |
| E29 | Control panel | No visible cue that a scored Connected piece has its own audio control Off: The Bloom played silent on the iPad with every Sound setting on, and only a URL in the app's log showed the Host sending `music=off`. A quiet mark on the piece's Rotation card, or on the collection's row, would have said so | HANDOFF §20 2026-09-16, the device round |
